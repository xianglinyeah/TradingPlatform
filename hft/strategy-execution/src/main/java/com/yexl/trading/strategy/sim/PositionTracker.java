...
public final class PositionTracker {
    ...
    public synchronized double avgCost() {
        return avgCost;
    }

    public synchronized double feesPaid() {
        return feesPaid;
    }
    ...
    public synchronized long partialFills() {
        return partialFills;
    }

    /** Reseeds state from a prior process's snapshot — call before any live fills are applied. */
    public synchronized void restore(double position, double avgCost, double realizedPnl,
                                      double feesPaid, long fills, long partialFills) {

...
public final class LatencyTracker {
     ...
     * Called periodically from the monitor thread. Each interval is also
     * folded into the per-segment cumulative histogram.
     */
    ...
    /**
     * Whole-run cumulative summary (everything folded in by
     * {@link #summaryLines()} so far) — the numbers a latency report quotes:
     * interval tables are too small a sample for a stable p99.9.
     */
    public List<String> cumulativeSummaryLines() {
        List<String> lines = new ArrayList<>(6);
        lines.add(exchangeToReceive.describeCumulative());
        lines.add(receiveToParse.describeCumulative());
        lines.add(parseToApply.describeCumulative());
        lines.add(receiveToApply.describeCumulative());
        lines.add(applyToPublish.describeCumulative());
        lines.add(receiveToPublish.describeCumulative());
        return lines;
    }
    ...
    private static final class Segment {
        ...
        /** Whole-run accumulation; only touched on the monitor thread. */
        private final Histogram cumulative =
                new Histogram(HIGHEST_TRACKABLE_NANOS, SIGNIFICANT_DIGITS);
        ...
        String describeInterval() {
            ...
            return describe(name, interval);
        }

        String describeCumulative() {
            return describe(name, cumulative);
        }

        private static String describe(String name, Histogram h) {
